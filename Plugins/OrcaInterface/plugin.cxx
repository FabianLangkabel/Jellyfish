...
#include "Nodes/ReadOrcaNode/ReadOrcaNode.hpp"
...
// our class plugin
class pluginOrcaInterface final : public micro::iplugin<>, public std::enable_shared_from_this<pluginOrcaInterface> {
public:

  plugin_node_manager* NodeManager;
  json plugin_infos;

  pluginOrcaInterface(int v, const std::string& nm):micro::iplugin<>(v, nm), std::enable_shared_from_this<pluginOrcaInterface>() {
    ...
    NodeManager->RegisteredNodes->registerClass<ReadOrcaNode>("ReadOrcaNode");
    ...
    subscribe<0>("GetPluginInfos", std::bind(&pluginOrcaInterface::GetPluginInfos, this));
  }

  ~pluginOrcaInterface() override {}
...
static std::shared_ptr<pluginOrcaInterface> instance = nullptr;
...
std::shared_ptr<micro::iplugin<>> import_plugin() {
  ...
  return instance ? instance : (instance = std::make_shared<pluginOrcaInterface>(plugin_version, "OrcaInterface"));
}