#include "TrotterPropagationQuESTNode.hpp"

TrotterPropagationQuESTNode::TrotterPropagationQuESTNode(std::string InpNodeTypeID) : Node()
{   
    NodeTypeID = InpNodeTypeID;
    NodeName = "Propagation with Trotter (QuEST)";
    NodeDescription = "Propagation with Trotter (QuEST)";
    NodeCategory = "Algorithms";
    Plugin = "QuantumComputing";
    HasCalulateFunction = true;
}

TrotterPropagationQuESTNode::~TrotterPropagationQuESTNode(){}

void TrotterPropagationQuESTNode::LoadData(json Data, std::string file)
{
...
json TrotterPropagationQuESTNode::SaveData(std::string file)
{
    if (State == NodeState::Calculated)
    {
...
void TrotterPropagationQuESTNode::calculate()
{
    QC::quantum_propagation_laser_cap_quest Propagator;
    ...
    {
        Propagator.set_initial_state_type(0);
        Propagator.set_number_qubits_to_flipp_initial(this->number_qubits_state1);
    }
    else
    {
        Propagator.set_initial_state_type(1);
        auto InitState = *(_InitState.lock().get());
        Propagator.set_initial_statevector(InitState);
    }
    if (this->dump_statevector)
    {
        ...
        for (int i = 0; i < AmpsToDump.size(); i++)
        {
            ...
            int decimalNumber = std::stoi(temp, 0, 2);
            /*
...
void TrotterPropagationQuESTNode::NodeInspector(QWidget* Inspector)
{
    ...
    connect(trotter_order_lineedit, &QLineEdit::textChanged, this, &TrotterPropagationQuESTNode::select_trotter_order);
    ...
    connect(trotter_timestep_lineedit, &QLineEdit::textChanged, this, &TrotterPropagationQuESTNode::select_trotter_timestep);
    ...
    connect(trotter_numbertimesteps_lineedit, &QLineEdit::textChanged, this, &TrotterPropagationQuESTNode::select_trotter_numbertimesteps);
    ...
    connect(use_lowest_flipped_init_wavefunction_box, &QCheckBox::stateChanged, this, &TrotterPropagationQuESTNode::select_use_lowest_flipped_init_wavefunction);
    ...
    connect(number_qubits_state1_lineedit, &QLineEdit::textChanged, this, &TrotterPropagationQuESTNode::select_number_qubits_state1);
    ...
    connect(dump_statevector_box, &QCheckBox::stateChanged, this, &TrotterPropagationQuESTNode::select_dump_statevector);
    ...
    connect(get_qite_expecation_values_without_measurement_checkbox, &QCheckBox::stateChanged, this, &TrotterPropagationQuESTNode::select_get_qite_expecation_values_without_measurement);
    ...
    connect(measurments_per_qite_expection_value_lineedit, &QLineEdit::textChanged, this, &TrotterPropagationQuESTNode::select_measurments_per_qite_expection_value);
...
bool TrotterPropagationQuESTNode::InPortAllowMultipleConnections(int Port)
{ 
    return false; 
}

unsigned int TrotterPropagationQuESTNode::Ports(NodePortType PortType)
{
...
bool TrotterPropagationQuESTNode::IsCustomPortCaption(NodePortType PortType, int Port)
{ 
    return true; 
}

std::string TrotterPropagationQuESTNode::CustomPortCaption(NodePortType PortType, int Port)
{ 
    if(PortType == NodePortType::In)
    {
        ...
        else if(Port == 3) {return "Statevector"; }
        else if(Port == 4) {return "CI Results"; }
    }
    else
    {
...
std::string TrotterPropagationQuESTNode::DataTypeName(NodePortType PortType, int Port)
{
    if(PortType == NodePortType::In)
    {
        ...
        else if(Port == 3) {return "QC::QC_Statevector"; }
        else if(Port == 4) {return "QC::CIResults"; }
    }
    else
    {
...
std::any TrotterPropagationQuESTNode::getOutData(int Port) 
{ 
    std::any ret;
    if(Port == 0)
    {
...
void TrotterPropagationQuESTNode::setInData(int Port, std::any data)
{
    if(data.has_value())
    {
        if(Port == 0)
    ...
    else
    {
        ...
        else if(Port == 3){_InitState.reset();}
        else if(Port == 4){_ci_results.reset();}
    }
}

void TrotterPropagationQuESTNode::select_trotter_order()
{
    this->trotter_order = this->trotter_order_lineedit->text().toInt();
    PropertiesChanged();
}

void TrotterPropagationQuESTNode::select_trotter_timestep()
{
    this->trotter_timestep = this->trotter_timestep_lineedit->text().toDouble();
    PropertiesChanged();
}

void TrotterPropagationQuESTNode::select_trotter_numbertimesteps()
{
    this->trotter_numbertimesteps = this->trotter_numbertimesteps_lineedit->text().toInt();
    PropertiesChanged();
}

void TrotterPropagationQuESTNode::select_use_lowest_flipped_init_wavefunction()
{
    this->use_lowest_flipped_init_wavefunction = this->use_lowest_flipped_init_wavefunction_box->isChecked();
    PropertiesChanged();
}

void TrotterPropagationQuESTNode::select_number_qubits_state1()
{
    this->number_qubits_state1 = this->number_qubits_state1_lineedit->text().toInt();
    PropertiesChanged();
}

void TrotterPropagationQuESTNode::select_dump_statevector()
{
    this->dump_statevector = this->dump_statevector_box->isChecked();
    PropertiesChanged();
}

void TrotterPropagationQuESTNode::select_get_qite_expecation_values_without_measurement()
{
...
void TrotterPropagationQuESTNode::select_measurments_per_qite_expection_value()
{
