...
namespace QC
{
	...
	public:
		...
		void set_laserfile(std::string Laserfile) { if (m_type == LaserType::File) { laser_file = Laserfile; } }
		bool use_file() {
		...
		std::string get_laserfile() {
			if (m_type == LaserType::File) { return laser_file; }
			else { return ""; }
		}
