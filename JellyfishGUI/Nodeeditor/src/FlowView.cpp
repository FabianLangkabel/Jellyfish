...
{
  /*
  auto& node = _scene->createNode("test", "Testnode");
...
{
  QStringList info = event->mimeData()->text().split("~");
  auto& node = _scene->createNode(info.at(0).toStdString(), info.at(1).toStdString());
  QPoint pos = event->pos();
  QPointF posView = this->mapToScene(pos);
  node.nodeGraphicsObject().setPos(posView);
  _scene->nodePlaced(node);
