...
NodeConnectionInteraction(GUINode& node, GUIConnection& connection, FlowScene& scene)
  : _node(&node)
...
bool NodeConnectionInteraction::canConnect(PortIndex &portIndex, TypeConverter & converter) const
{
  ...
  GUINode* node = _connection->getNode(oppositePort(requiredPort));
  ...
  std::string candidateNodeDataType = _node->dataType(requiredPort, portIndex);
...
tryConnect() const
{
  ...
  PortType requiredPort = connectionRequiredPort();
  _node->setConnection(requiredPort,
                                   portIndex,
                                   *_connection);
...
disconnect(PortType portToDisconnect) const
{
  ...
  _node->getEntries(portToDisconnect)[portIndex].erase(_connection->id());
...
{
  QPointF p = _node->portScenePosition(portIndex, portType);
                             ...
                             QPointF const & scenePoint) const
{
  ...
  PortIndex portIndex = _node->checkHitScenePoint(portType,
                                                    scenePoint,
                                                    sceneTransform);
  return portIndex;
}
...
{
  auto const & entries = _node->getEntries(portType);
  ...
  switch (portType)
  {
      case PortType::In:
      {
          const auto policy = _node->portInConnectionPolicy(portIndex);
          return policy == ConnectionPolicy::Many;
      }
      case PortType::Out:
      {
          const auto policy = _node->portOutConnectionPolicy(portIndex);
          return policy == ConnectionPolicy::Many;
      }
